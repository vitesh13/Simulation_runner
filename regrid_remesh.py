...
class Remesh_for_CA():
  ...
  def output_without_regridding(self,geom):
    """
    ...
    new_coords = np.stack(np.meshgrid(x_new,y_new,z_new,indexing='ij'),axis=-1).reshape(((nx)*(ny)*(nz),3),order='F')

    d = damask.Result(os.path.splitext(self.geom)[0] + os.path.splitext(self.load)[0] + '.hdf5')
    phase_name = d.phases 
    d.view('increments',f'inc{inc}')
    orientations = d.read_dataset(d.get_dataset_location('O'))
    ### grain rotation
    ...
    file_rg = 'remesh_%s_%s_inc%s.txt'%(os.path.splitext(self.geom),os.path.splitext(self.load),inc)
    if not os.path.exists(os.path.join('/nethome/v.shah',folder,'postProc')):
        os.makedirs(os.path.join('/nethome/v.shah',folder,'postProc'))
